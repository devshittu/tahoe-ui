...
export const createWizardStore = <TSteps extends WizardStep[]>(
  ...
  return create<WizardState<TSteps>>((set, get) => ({
    ...
    // Compute steps for rendering based on configuration
    // get renderedSteps() {
    //   const { currentStepIndex, visibleSteps } = get();
    ...
    //   if (renderAdjacent) {
    //     return visibleSteps.filter((_, index) =>
    //       [currentStepIndex - 1, currentStepIndex, currentStepIndex + 1].includes(index),
    //     );
    //   }

    //   return [visibleSteps[currentStepIndex]]; // Default: Render only current step
    // },
