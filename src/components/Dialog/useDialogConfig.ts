...
export function useDialogConfig(showFrom: DialogShowFrom) {
  ...
  const variants = {
    ...
    visible: { x: 0, y: 0, opacity: 1, transition: { type: "spring", bounce: 0 } },
    exit: (pos: DialogShowFrom) => {
      switch (pos) {
        case "bottom": return { y: "100%", opacity: 0, transition: { duration: 0.3 } };
        case "top": return { y: "-100%", opacity: 0, transition: { duration: 0.3 } };
        case "left": return { x: "-100%", opacity: 0, transition: { duration: 0.3 } };
        case "right": return { x: "100%", opacity: 0, transition: { duration: 0.3 } };
        default: return { y: "-100%", opacity: 0, transition: { duration: 0.3 } };
      }
    },
  };
